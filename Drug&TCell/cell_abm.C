...
void update_states(list<Cell> &Cells_local, list<Cell *> &Cells_tip, list<Cell> &Cells_vessel_start, list<Cell> &Cells_vessel_end, int time, Ran &ran, EquationSystems &equation_systems, const int outside_cells)
{
  ...
  std::unique_ptr<FEBase> sys_fe(FEBase::build(dim, sys_fe_type));
  ...
  for (it = Cells_local.begin(); it != Cells_local.end(); ++it)
  {
    ...
    if ((*it).state == 7)
    {
      ...
      if (Cells_tip.size() < max_tip_cells) // Just one tip cell TRYING TO STUDY THIS
        ...
        {
          if (sqrt(pow((*it).x - 0.5 * height, 2) + pow((*it).y - 0.5 * height, 2)) < 0.5 * height - 60)
          { // if not on boundary
            if (Cells_tip.size() > 0)
            {
              std::list<Cell *>::iterator it_p;
    ...
    //========== Tip Cell Time ==========
    if ((*it).state == 8||(*it).state == 16)
    {
      (*it).time += 1;
    }
    //========== Proliferative stalk cells ==========
    double drug_effect=1.5;
    if ((*it).state == 9)
    {
      if (tau >= (tau_E - tau_g1)*drug_effect)
      {
...
for (auto it = Cells_local.begin(); it != Cells_local.end(); ++it) {
        Cell &cell = *it;
        if (cell.state == 16) {
            for (auto jt = Cells_local.begin(); jt != Cells_local.end(); ) {
                Cell &tumor_cell = *jt;
                double dist = sqrt(pow(cell.x - tumor_cell.x, 2) + pow(cell.y - tumor_cell.y, 2));
                if (tumor_cell.state == 1 && dist < kill_dist) {
                    if (ran.doub() < kill_prob) {
                        jt = Cells_local.erase(jt);
                    } else {
                        ++jt;
                    }
                } else {
  ...
  while (iter < 500)
  {
    ...
    for (unsigned int bin_i = 0; bin_i < Cell_Bins.size(); bin_i++)
    {
      ...
      for (cell_a = Cell_Bins[bin_i].begin(); cell_a != Cell_Bins[bin_i].end(); ++cell_a)
      {
                  ...
                  if (jy + yy < number_bins && jy > 0)
                  {
                    ...
                    for (cell_b = Cell_Bins[bin_j].begin(); cell_b != Cell_Bins[bin_j].end(); ++cell_b)
                    {
                      ...
                      if ((*(*cell_b)).state == 9)
                      {
                        ...
                        if (d < ((*(*cell_a)).A_radius + (*(*cell_b)).A_radius))
                        { // && (*(*cell_b)).prev_state==(*(*cell_a)).prev_state){
                          if ((*(*cell_b)).x > (*(*cell_a)).x && (*(*cell_b)).x >= 150)
                          { // if stalk is closer to tumor skip first divide
                            (*(*cell_b)).state = 12;
        ...
        if ((*(*cell_a)).state == 8 || (*(*cell_a)).state == 9 || (*(*cell_a)).state == 11||(*(*cell_a)).state == 16)
        {
          ...
          //======== Removed 0.8 scale ==========
          if ((*(*cell_a)).anasto == 0||(*(*cell_a)).state == 16)
          {
            ...
            if (jy + vy[pxy] < number_bins && jy + vy[pxy] >= 0)
            {
              if (ix + vx[pxy] < number_bins && ix + vx[pxy] >= 0)
              {
                ...
                for (cell_b = cell_start; cell_b != Cell_Bins[bin_j].end(); ++cell_b)
                {
                  ...
                  { // if its not 13 and 14 theres adhesion
                    F_cca_i[0] += -c_cca * phi_x;
                    F_cca_i[1] += -c_cca * phi_y;
                  }
                  (*(*cell_a)).F[0] += (F_cca_i[0] + F_ccr_i[0]);
                  ...
                  if (((*(*cell_a)).state == 7 && (*(*cell_b)).state == 9) || ((*(*cell_b)).state == 7 && (*(*cell_a)).state == 9))
                  { // ernesto
                    /*
                  ...
                  if ((*(*cell_a)).state == 8 && ((*(*cell_b)).state == 12 || (*(*cell_b)).state == 9 || (*(*cell_b)).state == 11))
                  { // ernesto
                    double psi_vx = 0.0;
                    double psi_vy = 0.0;
                    if ((*(*cell_a)).anasto == 0)
...
void init_cond_cells(list<Cell> &Cells_local, const Parameters &all_parameters, Ran &ran)
{
 ...
 // const int num_immune_cells = all_parameters.get<int>("num_immune_cells");
  double pos[2];
  int number_t = 0;
  // int number_h = 0;
  double center[2];
  ...
  // Add immune cells
    ...
    for (int i = 0; i < num_immune; ++i) {
        Cell immune;
        double x = ran.doub() *400 + 400;
        double y = ran.doub() *400 + 300;
        immune.set(x, y, n_radius, c_radius, a_radius, 0.0, 0, 16);
        Cells_local.push_back(immune);
    }
}
