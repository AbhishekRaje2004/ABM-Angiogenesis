...
//  equation_systems.parameters.set<Real> ("nut_coeff")       = 0;
  ...
  do{
    ...
    *nutr.old_local_solution = *nutr.current_local_solution;
    ...
    std::ofstream file;
    string filename="cell_pop_data.csv";
    ...
    if (file.tellp() == 0)
    {
        file << "Time,Confluence,TotalCells,TumorCells,NecroticCells,QuiescentCells,ProliferativeCells,HypoxicCells,EndothelialCells,TipCells,StalkCells,OutsideCells\n";
    }

    // Write cell statistics to the file
    file << t_step << ","
         << confluence << ","
         << Cells_local.size() << ","
         << total_t << ","
